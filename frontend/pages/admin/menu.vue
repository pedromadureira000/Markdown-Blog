    ...
    <div>
      ...
      >
        <template v-slot:item.actions="{ item }">
          <menu-edit-menu :ref="'menu' + item.id" :menu="item" @menu-deleted="deleteMenu(item)" />
        </template>
        <template v-slot:item.icon="{ item }">
          <v-icon>{{item.icon}}</v-icon>
        </template>
        <template v-slot:item.default_submenu="{ item }">
          <p>{{item.default_submenu ? item.default_submenu.title : null}}</p>
        </template>
        </v-data-table>
      </div>
    </div>
