...
// -------------------------------/ user api
  ...
  async updateUser({commit, dispatch}: {commit: Commit, dispatch: Dispatch,}, payload: any){
    try {
    let data = await api.updateUser(payload)
    console.log(">>",data)
    commit("SET_USER", data )
    dispatch("setAlert", {message: "Your user has been updated.", alertType: "success"}, { root: true })
    }
    catch(e){
      handleError(e.response, commit)
      dispatch("setAlert", {message: "Something get wrong when trying to user.", alertType: "error"}, { root: true })
    }
  },
